...
- **Scope:** Pure structural decomposition only. Does **not** handle re-planning, risk assessment, or parallel approaches — those belong to the Strategist.

### B. Strategist

- **Goal:** Assess risk, manage escalation responses, and decide when to launch parallel approaches.
- **Trigger (escalation):** An `escalation.md` appears in a child directory, OR a child's status in the local `manifest.json` is `blocked`. This is the **upward feedback loop** — the Strategist restructures the plan after implementation hits a wall (see §3.3).
- **Trigger (risk):** After the Architect creates initial structure, the Strategist reviews the `manifest.json` and flags high-risk deliverables with `risk: "high"`.
- **Actions:**
  - **Re-plan:** On escalation, can restructure, re-route, or further escalate (see §3.3).
  - **Spawn parallel approaches:** For high-risk or previously-failed components, creates N competing candidate directories with `mode: "parallel"` in the parent `manifest.json` (see §3.4).
...
- **Goal:** Refine requirements and adapt to changes at a specific level of abstraction.
- **Trigger:** A sub-directory where `manifest.json` exists and all deliverables are `pending` (i.e., just initialized), OR a `change_request.md` from the Strategist or a higher level.
...
- **Action:** Writes source code as specified by the local `prd.md`, deposits it into the directory, and updates the deliverable's status to `in_progress` in `manifest.json`.

### E. Code Reviewer / QA

- **Goal:** Ensure code meets the local PRD requirements.
- **Trigger:** A deliverable in `manifest.json` has status `in_progress` and the corresponding file exists.
- **Action:** Compares the code against the local `prd.md` and runs tests.
...
A master `prd.md` is placed in `/project` (e.g., "Build a snake game"). The Architect creates `/ui` and `/logic` folders, each with a skeletal `prd.md` and a `manifest.json`. The Strategist reviews for risk flags. The PM expands the PRDs and populates the manifests. The Engineer reads the leaf-node PRDs, writes `render.py` in `/ui` and `engine.py` in `/logic`, and marks them `in_progress` in the local manifests.
...
3. **Escalation:** If the PM's simplification still fails, or if the failure is structural (e.g., a dependency between sibling directories is impossible), the QA/PM agent writes `escalation.md` in the failing directory. This file includes:
...
4. **Strategist re-plan:** The parent directory's Strategist detects `escalation.md` (or sees `blocked` status in its `manifest.json`). It can now:
   ...
   - **Launch parallel retry:** Spawn redundant competing approaches for the failed component (§3.4).
   - **Escalate further:** If the Strategist at this level cannot resolve it, it writes its own `escalation.md`, propagating the signal to *its* parent. This recursive escalation continues until the problem reaches a level with enough authority to resolve it.

### 3.4 Redundant Parallel Approaches and Voting

Inspired by ant colonies where multiple foragers explore redundant paths to cover failure, the Strategist can launch **parallel competing approaches** for any key step. This trades compute cost for resilience.

#### Spawning (Strategist)

When the Strategist identifies a component as high-risk (ambiguous requirements, unknown feasibility, or after a first-attempt escalation), it creates N sibling directories for the same deliverable:
...
Each candidate directory is a **fully independent branch** — it gets its own `prd.md`, `manifest.json`, and runs through the normal PM → Engineer → QA pipeline in parallel. The Strategist can vary:
...
3. If **no candidate passes**, this counts as a structural failure and triggers escalation (§3.3) — the Strategist re-plans the requirement.

#### When to Use Parallel Approaches

Not every step warrants redundancy. The Strategist spawns parallel candidates when:
