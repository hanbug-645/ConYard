...
def get_orchestrator_config(config: Optional[dict] = None) -> dict:
    if config is None:
        config = load_config()
...
def get_architect_config(config: Optional[dict] = None) -> dict:
    if config is None:
        config = load_config()
    return config.get("architect", {
        "max_files_per_dir": 6,
        "max_subdirs_per_dir": 5,
