...
class FireAntServer:
    ...
    def _handle_do(self, task: str):
        ...
        # Create project directory with bootstrapped files (kaplay, index.html, package.json)
        project_name = self._generate_project_name(task)
        project_dir = self.workspace_root / "projects" / project_name
        from .main import bootstrap_project
        bootstrap_project(project_dir, title=project_name)
        
        self.current_project = project_dir
        
        # Give Manager an operation logger for this project
        self.manager.set_operation_logger(OperationLogger(project_dir))

        # Manager creates _prd.md directly
        print("📝 Creating PRD...")
    ...
    def _handle_ask(self, question: str):
        ...
        try:
            if not self.manager.operation_logger:
                self.manager.set_operation_logger(OperationLogger(self.current_project))
    ...
    def _handle_status(self):
        ...
        # Check project completion via manifest status
        from .utils.manifest import all_passed
        if all_passed(self.current_project):
            print(f"   Status: ✅ Complete")
        elif (self.current_project / "_escalation.json").exists():
            print(f"   Status: ⚠️  Escalated")
        elif (self.current_project / "_manifest.json").exists():
    ...
    def _generate_readme(self, project_dir: Path):
        ...
        prd_content = (project_dir / "_prd.md").read_text() if (project_dir / "_prd.md").exists() else ""
        ...
        for item in project_dir.rglob("*"):
            if item.is_file() and not item.name.startswith(".") and not item.name.startswith("_") and item.name != "README.md":
