...
from ..utils.manifest import STANDARD_FILES, all_passed, load_manifest
...
class TreeScanner:
    ...
    def _classify(self, directory: Path) -> str:
        ...
        # No manifest and no signal files → not a project component, skip
        if not has_manifest and not has_prd:
            return self.COMPLETE

        # Terminal: all deliverables reached qa_pass
        if has_manifest and all_passed(directory):
            return self.COMPLETE
        ...
        # No agent triggered but dir isn't complete — waiting for children/dependencies
        return self.WAITING
